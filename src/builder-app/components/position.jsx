...
import { selectSelectedPath, selectTree, setPosition } from "../store/treeSlice";
import { stripPx } from "../utils";
import { PxDragInput } from ".";
...
export function PositionControl({ selectedNode }) {
    ...
    const [offset, setOffset] = useState(stripPx(currentOffset));

    useEffect(() => {
        setOffset(stripPx(currentOffset));
    ...
    return (
        <div className="dg_bd_layout_edit_tool_wrapper">
            ...
            <div className="dg_bd_layout_edit_tool_wrapper">
                <div className="dg_bd_layout_edit_tool_label">offset</div>

                <div className="dg_bd_layout_edit_tool_wrapper_variants">
                    <PxDragInput value={offset} onChange={(v) => setOffset(v)} min={0} max={64} />
                </div>
            </div>
        </div>
    );
